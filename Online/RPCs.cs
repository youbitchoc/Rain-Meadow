    ...
    public static class RPCs
    {
        ...
        public static void ChangeFood(short amt)
        {
            if (RWCustom.Custom.rainWorld.processManager.currentMainLoop is RainWorldGame game && game.Players[0]?.state is PlayerState state)
            {
                ...
                RainMeadow.Debug($"amt:{amt} new:{newFood} after:{state.foodInStomach*4}+{state.quarterFoodPoints}={state.foodInStomach * 4 + state.quarterFoodPoints}");
            }
        }
        ...
        public static void KickToLobby()
        {
            ...
            if (manager.musicPlayer != null) manager.musicPlayer.DeathEvent();

            manager.RequestMainProcessSwitch(RainMeadow.Ext_ProcessID.LobbySelectMenu);
            BanHammer.ShowBan(manager);
        }
